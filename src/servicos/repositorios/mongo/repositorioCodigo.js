...
module.exports = conexao => ({
  /**
   * Obtém codigo pelo id.
   * @function {obter}
   *
   * @param {Object} dadosCodigo
   * @param {string} dadosCodigo.id - id do codigo para busca.
   *
      ...
      if (dadosRetorno.status.codigo === 200) {
        ...
        if (r) {
          ...
          if (cc.dados)
            return retorno(
              200,
              "codigos recuperados com sucesso.",
              {},
              { codigo: cc.dados }
            );
          else
            return retorno(
              400,
              "codigo com dados inconsistentes.",
              {},
              {},
              cc.erros
            );
   ...
   * @returns {object} {status: {codigo: number, mensagem: string}, codigo: Object}.
   */
  obterPorTitulo: async function(dadosCodigo) {
    const validarParametros = dadosCodigo => {
  ...
  gravar: async function(dadosCodigo) {
    ...
    const inserirCodigo = async dadosCodigo => {
      try {
        ...
        console.error(
          "[ERRO]",
          "[servicos.repositorios.mongo.repositorioCodigo.gravar.inserirCodigo]",
          `[${err.message}]`,
          err
        );
      ...
      const r = await conexao.collection("Codigos").updateOne(
        { id: dadosCodigo.id },
        {
          $set: {
            ...
            favoritos: dadosCodigo.favoritos
          }
        }
      );
    ...
    const gravarCodigo = async retornoCodigo => {
      if (retornoCodigo.dados) {
        ...
        return retorno(
          400,
          "dados inconsistentes no codigo",
          {},
          {},
          retornoCodigo.erros
        );
  ...
  apagar: async function(dadosCodigo) {
    ...
    const apagarCodigo = async dadosRetorno => {
      if (dadosRetorno.status.codigo === 200) {
        ...
        if (r.deletedCount > 0) return retorno(200, "codigo apagado.");
