...
module.exports = repositorioCodigo => ({
  ...
  validarParametro: function(parametro) {
    if (
      ...
      return retorno(
        400,
        "parâmetro inválido: { autor: string, titulo: string, descricao: string, tecs: [strings], id: string }"
      );
   ...
   * @param {Object} dadosRetorno.dadosCodigo - objeto com informações do usuario e codigo para favoritar.
   *
  ...
  gerarCodigo: function(dadosRetorno) {
    ...
    if (dadosCodigo.dados)
      return retorno(200, "codigo gerado.", dadosRetorno, {
        codigo: dadosCodigo.dados
      });

    return retorno(
      400,
      "os dados para gravar o codigo estão inconsistentes",
      dadosRetorno,
      {},
      dadosCodigo.erros
    );
