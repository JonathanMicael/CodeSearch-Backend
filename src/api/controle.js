...
module.exports = interfaces => ({
  ...
  executar: async (nomeMetodo, req, res) => {
    const requisicoes = {
      ...
      /**
       * Requisição POST para favoritar um Codigo.
       * @param {object} req - requisição do express.
       * @returns {Object} {status: {codigo: number, mensagem: string}, dados: Object}.
       */
      favoritarCodigo: async req => {
        const { usuario_id } = req.login;
        const { id } = req.params;
        const r = await interfaces.entradaComandos.executar("favoritarCodigo", {
          usuario_id,
          codigo_id: id
        });
        return r;
      ...
      /**
       * Requisição POST para desfavoritar um Codigo.
       * @param {object} req - requisição do express.
       * @returns {Object} {status: {codigo: number, mensagem: string}, dados: Object}.
       */
      desfavoritarCodigo: async req => {
        const { usuario_id } = req.login;
        const { id } = req.params;
      ...
      /**
       * Requisição GET para obter um codigo.
       * @param {object} req - web req data
       * @returns {Object} {status: {codigo: number, mensagem: string}, dados: object}.
       */
       ...
       * @returns {Object} {status: {codigo: number, mensagem: string}}.
       */
      apagarUsuario: async req => {
        const { usuario_id, permissoes } = req.login;
        const { id } = req.params;
        const r = await interfaces.entradaComandos.executar("apagarUsuario", {
          id,
          usuario_id,
          permissoes
        });
    ...
    const autorizacoes = {
      /**
       * Autorização na requisição POST para favoritar o codigo.
      ...
      apagarUsuario: req => {
        if (req.login) {
          if (
            req.login.permissoes.includes("usuario") ||
            req.login.permissoes.includes("admin")
          ) {
            return retorno(200, "usuário autorizado para apagar o usuario.");
          }
        }
        return retorno(403, "usuário não autorizado para apagar o usuario.");
      },
      /**
       * Autorização na requisição POST para favoritar o codigo.
       * @param {Request} req - requisição do express.
      ...
      favoritarCodigo: req => {
        if (req.login) {
          if (
            req.login.permissoes.includes("usuario") ||
            req.login.permissoes.includes("editor") ||
            req.login.permissoes.includes("admin")
          ) {
            return retorno(200, "usuário autorizado para favoritar codigo.");
          }
        }
        return retorno(403, "usuário não autorizado para favoritar codigo.");
      },
      /**
       * Autorização na requisição POST para desfavoritar o codigo.
       * @param {Request} req - requisição do express.
       * @returns {Object} {status: {codigo: number, mensagem: string}}.
       */
      desfavoritarCodigo: req => {
        if (req.login) {
          if (
            req.login.permissoes.includes("usuario") ||
            req.login.permissoes.includes("editor") ||
            ...
            return retorno(200, "usuário autorizado para desfavoritar codigo.");
          }
        }
        return retorno(403, "usuário não autorizado para desfavoritar codigo.");
      },
      /**
    ...
    const executarAutorizacao = (req, autorizacoes, nomeMetodo) =>
      autorizacoes[nomeMetodo](req);

    const executarRequisicao = (
      req,
      requisicoes,
      nomeMetodo
    ) => async entrada => {
      if (entrada.status.codigo === 403) return entrada;
