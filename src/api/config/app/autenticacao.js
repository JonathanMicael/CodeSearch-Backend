...
module.exports = (conexao) => (app) => {
    ...
    const autenticador = (req, res, next) => {
        ...
        const validarToken = (entrada) => {
          if (entrada.erro || entrada.pathLiberado)
             return entrada;
        ...
        const validarAutorizacao = async (entrada) => {
            ...
            const r = await conexao.collection('Usuarios').findOne({ id: entrada.dados });

            if (r) {
                return {
                    erro: '',
                    dados: { permissoes: r.permissoes },
                    pathLiberado: entrada.pathLiberado
                }
            }
            return {
        ...
        const autenticar = compose(then(injetarDadosLogin), validarAutorizacao, validarToken, pegarCabecalhoAutorizacao, verificarPathLiberado)
        autenticar().then(r => {
            if (r.erro)
                res.status(401).json(retornoAPI(401, r.erro));
            else
                next();
        });
